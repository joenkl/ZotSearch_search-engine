...
module.exports.calculateRankingScore = (
  ...
  avgTotalTagScore /= countSumFre;
  if (avgTotalTagScore != 0) {
    avgTotalTagScore = log10(avgTotalTagScore);
  }

  avgHighestTagScore /= nunTermInRankInfo;
  avgHighestTagScore = avgHighestTagScore / 7;

  let avgNumOfMatchWords = 0;
  if (numOfMatchWords != 0) {
    avgNumOfMatchWords = log10(numOfMatchWords * nunTermInRankInfo);
  }
  ...
  finalScore =
    0.3 * avgTFIDF +
    0.35 * avgHighestTagScore +
    0.1 * avgTotalTagScore +
    0.25 * avgNumOfMatchWords;
